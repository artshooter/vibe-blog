...
export default function Hero({ inHome = false }: HeroProps) {
  ...
  const content = (
    <motion.div
      className={`relative w-full ${inHome ? 'h-[400px]' : 'min-h-[60vh]'} bg-[#FDFBF7] overflow-hidden ${inHome ? 'cursor-pointer group' : ''}`}
      initial={{ opacity: 0 }}
      animate={{ opacity: 1 }}
    ...
    >
      ...
      <div className={`relative z-10 h-full flex flex-col justify-center items-center px-8 text-center ${inHome ? '' : 'py-20'}`}>
        ...
        >
          <span className="text-[#81B29A] text-sm tracking-[0.3em] font-light">
            {t('hero.year')}
          </span>
        </motion.div>
        ...
        {/* 主标题 */}
        <motion.h1
          className={`${inHome ? 'text-3xl md:text-4xl' : 'text-4xl md:text-5xl'} text-[#3D3D3D] tracking-wide mb-4 font-light`}
          initial={{ y: 20, opacity: 0 }}
          animate={{ y: 0, opacity: 1 }}
        ...
        {/* 副标题 */}
        <motion.p
          className={`max-w-md ${inHome ? 'text-sm md:text-base' : 'text-base md:text-lg'} text-[#3D3D3D]/70 leading-relaxed font-light`}
          initial={{ y: 20, opacity: 0 }}
          animate={{ y: 0, opacity: 1 }}
        ...
        {inHome && (
          <motion.div
            className="absolute bottom-6 left-1/2 -translate-x-1/2 text-sm text-[#3D3D3D]/40 opacity-0 group-hover:opacity-100 transition-opacity duration-300"
          >
            {t('hero.clickToRead')}
          </motion.div>
        )}
